...
a_content => a_dictionary%get(key=3_int32)
...
a_content => a_dictionary%get(key=5_int32)
if (associated(a_content)) then
  select type(a_content)
  type is(integer(int32))
...
a_content => a_dictionary%get(key=3_int32)
another_content => a_dictionary%get(key=3_int32)
...
test_passed(9) = len(a_dictionary)==0
print "(A,L1)", 'a_dictionary destroyed, is correct? ', test_passed(9)
